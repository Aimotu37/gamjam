...
public class S7StateController : MonoBehaviour
{
    ...
    [Header("1:Intro 2:Reading 3:BeforeSmash 4:AfterSmash 5:End")]
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha3)) Trigger(RoomState.S7_BeforeSmash);
        if (Input.GetKeyDown(KeyCode.Alpha4)) Trigger(RoomState.S7_AfterSmash);
        if (Input.GetKeyDown(KeyCode.Alpha5)) Trigger(RoomState.S7_End);
    }

    private void Trigger(RoomState target)
    {
        var gm = S7.GameManager.Instance;
        if (gm == null) { Debug.LogError("[S7StateController] Instance 未初始化"); return; }
