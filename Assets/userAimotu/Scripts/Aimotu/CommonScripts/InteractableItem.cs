...
public class InteractableItem : MonoBehaviour
{
    ...
    private IGameManager GetManager()
    {
       /* if (S3.GameManager.Instance != null) return (IGameManager)S3.GameManager.Instance;
        ...
        return manager;*/
        // 直接找当前场景里的 SceneManagerBase，不依赖各 S* 的 static Instance
        // （static 字段可能持有上个场景被销毁但未清空的引用，导致链式返回 null）
        return Object.FindAnyObjectByType<SceneManagerBase>() as IGameManager;
    }
    ...
    {
        Debug.Log($"[Trigger] {gameObject.name} 检测到碰撞：{other.gameObject.name} tag={other.tag}");
    ...
    public void RefreshInteractable()
    {
        ...
        Debug.Log($"[Refresh] {gameObject.name} nearby={isPlayerNearby} eligible={CheckStateEligibility()} canInteract={canInteract}");
